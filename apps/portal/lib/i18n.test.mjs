/**
 * Guards the customer portal's Arabic dictionary. Same checks as the staff
 * portal's, for the same reason: a key added in English and forgotten in Arabic
 * looks completely fine to whoever added it, and is only noticed by the person
...
test('no Arabic string is left as English', () => {
  ...
  // `nav.lang` names the OTHER language and is written in it — that is the
  // convention, not an oversight. `pt.yourThing` is a bare placeholder: the
  // reward's own name is interpolated into it.
  const allowed = new Set(['nav.lang', 'pt.yourThing']);
...
test('Arabic noun phrases stay bound with non-breaking spaces', () => {
  ...
  // Terms only — one concept each. Demonstratives and prepositions break
  // freely in Arabic and are deliberately absent.
  const BOUND = [
    ['كلمة', 'المرور'],
    ['البريد', 'الإلكتروني'],
    ['رقم', 'الجوال'],
